...
import { createBlog, updateBlog } from "@abhilashdash/medium-common";
...
blog.use("/*", async (c, next) => {
  //take the token from the user from header and return the authorId
...
blog.post("/", async (c) => {
  ...
  try {
    ...
    c.status(411);
    return c.json({ msg: "Give correct data types"});
    }
  } catch (error) {
    c.status(411);
...
blog.put("/", async (c) => {
  ...
  if(!success) { c.status(413); return c.json({msg:"Give correct data types"})};
  try {
